...
for i in range(1,count,1):
    ...
    player_skill_list:int = df.loc[i,1:5].to_list()
    player_rank_list:int = df.loc[i,7:11].to_list()
    ...
    # 新しい評価を計算する
    rating_groups  =  [( r1 ,),  ( r2 ,),  ( r3 ,),  ( r4 ,),  ( r5 ,)]
    # print(rating_groups) 
    rated_rating_groups  =  env . rate ( rating_groups ,  ranks = player_rank_list) 
    # 新しい評価を保存します
    (r1 ,),  ( r2 ,) ,  ( r3 ,),  ( r4 ,),  ( r5 ,) =  rated_rating_groups
    #output_df用のリストを作成する。
    output_list = []
    mu_sigma_list = [r1.mu,r1.sigma,r2.mu,r2.sigma,r3.mu,r3.sigma,r4.mu,r4.sigma,r5.mu,r5.sigma,'-','-']
...
#csvに出力
output_df.to_csv('5_player_output.csv',index=False)
print(output_df)
